...
import { AccountTabs, ActiveTab } from "./components/account-tabs";

import { EmailCard, PasswordCard, DangerZone } from "./components/action-cards";
...
export default function AccountPage() {
    ...
    const handleAvatarUpload = async (file: File) => {
        ...
        try {
            ...
            if (updateError) { setError("Failed to update profile picture"); return; }
    ...
    const handleRemoveAvatar = async () => {
        ...
        try {
            ...
            if (error) { setError("Failed to remove profile picture"); return; }
            setAvatarUrl(null);
    ...
    const handleChangeEmail = async () => {
        if (!newEmail.trim() || !emailPassword) { setError("Please fill in all fields"); return; }
        setIsSaving(true);
        setError(null);
        try {
            const { error: signInError } = await supabase.auth.signInWithPassword({
                email: userEmail, password: emailPassword,
            });
            if (signInError) { setError("Incorrect password"); setIsSaving(false); return; }
    ...
    const handleChangePassword = async () => {
        ...
        try {
            ...
            if (signInError) { setError("Current password is incorrect"); setIsSaving(false); return; }
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            const { error: signInError } = await supabase.auth.signInWithPassword({
                email: userEmail, password: deletePassword,
            });
            if (signInError) { setError("Incorrect password"); setIsSaving(false); return; }
            ...
            if (!response.ok) { setError("Failed to delete account. Please try again."); setIsSaving(false); return; }
