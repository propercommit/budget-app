...
- **Do money arithmetic in integer minor units, never in floats.** This app stores money as Prisma `Float` (`SpendingItem.budgeted`/`spent`, `IncomeSource.amount`, `SpendingEntry.amount`) — that is legacy storage, **not** a pattern to extend. IEEE-754 can't represent most decimal fractions, so summing amounts drifts and equality/threshold checks fail on clean data (`0.1 + 0.2 !== 0.3`). Rules for new code:
  ...
  - **Convert cents → major units only at the boundary** (display, or when writing into the existing `Float` columns), never in the arithmetic. When you must convert a decimal string to cents, use `Math.round(x * 100)` — never `Math.floor`/`parseInt` (drops a cent on `0.29`/`4.10`/`8.20`).
  - `formatAmount` (`lib/utils.ts`) rounds via `toLocaleString`/`toFixed`, so float drift is **invisible in the UI** — a passing visual check does not mean the underlying number is right. The unmasked risk is raw-float comparisons (`spent > budget`, `remaining >= 0`); prefer integer minor units there too.
  - Migrating the `Float` columns to integer `Int` cents is the intended long-term fix (DB migration + `round(value*100)` on existing rows + ~20–25 call sites + updating the intentional drift assertions in `lib/__tests__/utils.test.ts`). Until then, don't let floats leak into new summed/compared code.
