...
- `DATABASE_URL` — Prisma runtime AND migrations. Points at the Supavisor **session-mode** pooler (port 5432, no `pgbouncer=true`), which supports running migrations; no separate direct connection is configured.
- There is **no `DIRECT_URL` in `.env`**, even though `schema.prisma` declares `directUrl = env("DIRECT_URL")` — Prisma migrate/validate commands fail with P1012 unless it is exported. When running migrations, export it set to the same session-pooler URL as `DATABASE_URL`.
- `SUPABASE_JWT_PUBLIC_KEY` — ES256 JWK JSON used by `lib/auth.ts` to locally verify Supabase JWTs
- `REDIS_URL` — Upstash Redis. Backs live token revocation: the `revoked-before` epoch (password reset) and `recovery-session` containment. `getAuthenticatedUser` reads it on every request, so the app hard-depends on Redis for auth. (The separate `blockToken` blocklist is inert — see Auth notes.)
- Previously listed here but **not** actually in `.env`: `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `SITE_PASSWORD` (presumably deployment-only).
...
- Migrations: 10 in `prisma/migrations/`, latest `20260714180354_drop_receipt_url` — the CONTRACT half of the receipts expand/contract pair (`20260714180322_add_receipt_storage_columns` is the expand half). ⚠️ **Do not `prisma migrate deploy`/`dev` the contract step against the shared hosted DB until the `main` deployment no longer selects `receiptUrl`** (see the migration.sql comment), and never accept a drift-reset prompt — the DB is shared with production. Earlier `20260707000000_budget_series` was destructive by design (applied via reset on an empty DB, no backfill). `migration_lock.toml` pins `postgresql`. Migrations run against `DIRECT_URL`, which is **not** in `.env` — export it as the session-pooler `DATABASE_URL` value (see Environment); runtime uses `DATABASE_URL`.
...
2. Loads data: **`spendingItems` is awaited first** (capped to a 12-month window), then categories + current-month income + cross-month income + pre-window entry counts run in `Promise.all`. Cross-month income (`allIncomeSources`, drives trends) is loaded for the **same 12-month window** as the items (`month: { gte: cutoffMonth }`) — income-only months chart too; the trend month axis is the union of spending buckets and income months (`lib/trend-months.ts`). `preWindowEntryCounts` (per-category entry counts older than the window) feeds Manage Categories' delete warnings.
...
- **Receipts live in Supabase Storage** (private `receipts` bucket, one object per entry at the fixed key `<userId>/<entryId>` — layout constants in `lib/receipt-storage.ts`). The entry row carries only `receiptPath` (presence marker) + `receiptSizeBytes` (confirm-written, drives a 50 MB/user cap). Upload is signed-direct (POST `/api/entries/[id]/receipt` mints an `upsert: true` token, the browser uploads straight to Storage, PUT confirms with magic-byte sniff + metadata-size checks — the file never crosses a Vercel function). Images are compressed client-side via `browser-image-compression` (`lib/compress-image.ts`: maxSizeMB 1, maxWidthOrHeight 1920) with a raw fallback for allowlisted images whose compression fails (`lib/receipt-file.ts`); jpeg/png/webp only, end-to-end (bucket MIME array + server sniff + client allowlist change together). Rendering fetches short-lived signed URLs on popin open (`use-receipt-url`); `ReceiptViewer` downloads via blob-fetch (extension from the blob type — the anchor `download` attr is a cross-origin no-op).
